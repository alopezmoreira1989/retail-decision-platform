...
**"Stock available at the start of day" is an external input, not designed in this document.** Document 8 (Inventory) owns everything about how that quantity comes to be — depletion, replenishment, lead times. This document only defines the shape of the relationship demand and stock enter into to produce sales, the same way Document 5 treated `physical_inventory` as an input owned entirely by Document 8 without needing to resolve it. This is a real, acknowledged forward dependency, not a gap being glossed over: Document 7 comes before Document 8 in the roadmap, but conceptually needs one quantity from it. That's fine as long as the dependency is named, not hidden.
